...
INSERT INTO products (seller, title, content, price, stock, medias, created_at) VALUES ($1, $2, $3, $4, $5, $6, $7) RETURNING *;
...
SELECT products.*, users.email, users.phone, users.profile FROM products JOIN users ON products.author = users.username WHERE products.id = $1 LIMIT 1;

-- name: GetProductsWithSeller :many
SELECT products.*, users.email, users.phone, users.profile FROM products JOIN users ON products.author = users.username ORDER BY products.created_at DESC LIMIT $1 OFFSET $2;
